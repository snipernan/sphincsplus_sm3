...
// Performs sm3x8 on an initialized (and perhaps seeded) state.
static void _sm3x8(
              sm3ctx *ctx,
              unsigned char *out0,
              unsigned char *out1,
    ...
    sm3_final8x(ctx, out0, out1, out2, out3, out4, out5, out6, out7);
}
...
void sm3x8_seeded(
    ...
    _sm3x8(&ctx, out0, out1, out2, out3, out4, out5, out6, out7,
            ...
            unsigned long inlen)
{
    ...
    for (i = 0; (i+1)*SPX_SM3_OUTPUT_BYTES <= outlen; i++) {
        ...
        outx8 += SPX_SM3_OUTPUT_BYTES;
    }
    /* Until we cannot anymore, and we fill the remainder. */
    for (j = 0; j < 8; j++) {
        u32_to_bytes(inbufx8 + inlen + j*(inlen + 4), i);
    }
    sm3x8(outbufx8 + 0*SPX_SM3_OUTPUT_BYTES,
    ...
    for (j = 0; j < 8; j++) {
        memcpy(outx8 + j*outlen,
               outbufx8 + j*SPX_SM3_OUTPUT_BYTES,
               outlen - i*SPX_SM3_OUTPUT_BYTES);
    }
}
